...
python3 <(curl http://public.mckelvie.org/vpyapp.py) \
  -v --tb run --update git+https://github.com/sammck/xpulumi-installer.git@stable \
  xpulumi-installer --tb install --package git+https://github.com/sammck/xpulumi.git@stable
